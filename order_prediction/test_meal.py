...
def plot_swarm(rf_resids, gb_resids, base_resids):
	## Converting residual information into dictionary for Swarm Plot
	length = len(rf_resids*4)
	models = ['Random Forest']*length + ['Gradient Boosted']*length + ['Base (.25)']*length
	...
	plt.vlines(np.mean(gb_rs), .2, 1.8, linestyles='--', color='mediumseagreen', label="Mean {0:.3f}".format(np.mean(gb_rs)))
...
if __name__ == '__main__':
	...
	plot_swarm(rf_resids, gb_resids, base_resids)
